...
const TRAIN_START = WINDOW_SIZE;
const TRAIN_END = 259;
const EVALUATE_START = 260;
$EVALUATE_END = count(SUNSPOTS)-1;
const MAX_ERROR = 0.1;

$normalized_sunspots = [];
$closed_loop_sunspots = [];

function normalize_sunspots(float $high, float $low) {
	global $normalized_sunspots, $closed_loop_sunspots;

	$normalizer = new NormalizeArray();
	$normalizer->setNormalizedHigh($high);
	$normalizer->setNormalizedLow($low);

	$normalized_sunspots = $normalizer->process(SUNSPOTS);
	$closed_loop_sunspots = $normalized_sunspots;
}

function generate_training(): MLDataSet {
	global $normalized_sunspots;

	$result = new TemporalMLDataSet(WINDOW_SIZE, 1);
	$desc = new TemporalDataDescription(TemporalDataType::$RAW, true, true);
	$result->addDescription($desc);

	for ($year = TRAIN_START; $year < TRAIN_END; $year++) {
		$point = new TemporalPoint(1);
		$point->setDataAt(0, $normalized_sunspots[$year]);
		$point->setSequence($year);
		$result->getPoints()[] = $point;
	}
	$result->generate();
	return $result;
}

function create_network(): BasicNetwork {
	$network = new BasicNetwork();
	$network->addLayer(BasicLayer::create(WINDOW_SIZE));
...
function train(BasicNetwork $network, MLDataSet $training) {
	$train = new ResilientPropagation($network, $training);

	do {
		echo "Epoch #{$train->getIteration()} Error: ";
		$train->iteration();
		echo $train->getError(), PHP_EOL;
	} while ($train->getError() > MAX_ERROR);
}

normalize_sunspots(0.9, 0.1);
