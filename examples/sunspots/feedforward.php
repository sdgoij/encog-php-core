...
class SunspotPredictor {
	const STARTING_YEAR = 1700;
	const MAX_ERROR = 0.001;
	const WINDOW_SIZE = 30;
	const TRAIN_START = 30;
	const TRAIN_END = 259;
	const EVAL_START = 260;

	public function __construct(float ...$samples) {
		$normalizer = new NormalizeArray();
		$normalizer->setNormalizedHigh(0.9);
		$normalizer->setNormalizedLow(0.1);

		$this->samples = $normalizer->process($samples);
		$this->loop = $this->samples;
	}

	public function run() {
		$network = $this->createNetwork();
		$training = $this->generateTraining();
		$trainer = new ResilientPropagation(
			$network, $training
		);
		...
		do {
			$trainer->iteration();
			echo "Epoch #{$trainer->getIteration()} Error: {$trainer->getError()}\n";
		} while ($trainer->getError() > self::MAX_ERROR);

		$this->predict($network);
	}

	private function predict(BasicNetwork $network) {

		echo "Year\tActual\tPredict\tClosed Loop Predict\n";

		$format = function (float $v, $n = 4): string {
			return sprintf("%1.0{$n}f", $v);
		};

		for ($year = self::EVAL_START; $year < count($this->samples); $year++) {
			$input = new BasicMLData(self::WINDOW_SIZE);
			for ($i = 0; $i < $input->size(); $i++) {
				$input->setDataAt($i, $this->samples[($year-self::WINDOW_SIZE)+$i]);
			}
			$output = $network->compute($input);
			$prediction = $output->getDataAt(0);
			$this->loop[$year] = $prediction;

			for ($i = 0; $i < $input->size(); $i++) {
				$input->setDataAt($i, $this->loop[($year-self::WINDOW_SIZE)+$i]);
			}
			$output = $network->compute($input);
			$closedLoopPrediction = $output->getDataAt(0);

			echo self::STARTING_YEAR+$year, "\t",
				$format($this->samples[$year]), "\t",
				$format($prediction), "\t",
				$format($closedLoopPrediction), "\n";
		}
	}

	private function createNetwork(): BasicNetwork {
		$network = new BasicNetwork();
		$network->addLayer(BasicLayer::create(self::WINDOW_SIZE));
	...
	private function generateTraining(): MLDataSet {
		$temporal = new TemporalMLDataSet(self::WINDOW_SIZE, 1);
		$temporal->addDescription(new TemporalDataDescription(
			TemporalDataType::Raw(), true, true)
		);

		for ($i = self::TRAIN_START; $i < self::TRAIN_END; $i++) {
			$point = new TemporalPoint(1);
			$point->setSequence($i);
			$point->setDataAt(0, $this->samples[$i]);
			$temporal->getPoints()[] = $point;
		}
...
$p = new SunspotPredictor(...SUNSPOTS);
$p->run();
