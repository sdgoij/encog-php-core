...
class GaussianFunctionTest extends TestCase {
	public function testCalculateSingleDimensional() {
		...
		$this->assertEquals(0.8824969025845955, $rbf->calculate([1,2,3]));
		$this->assertEquals(0.04393693362340742, $rbf->calculate([3,2,1]));
		$this->assertEquals(0.8824969025845955, $rbf->calculate([1]));
		$this->assertEquals(0.04393693362340742, $rbf->calculate([3]));
	...
	public function testCalculateZeroCentered() {
		...
		$this->assertEquals(0.011108996538242306, $rbf->calculate([3,2,1]));
		$this->assertEquals(0.6065306597126334, $rbf->calculate([1]));
		$this->assertEquals(0.1353352832366127, $rbf->calculate([2]));
		$this->assertEquals(0.011108996538242306, $rbf->calculate([3]));
		$this->assertEquals(1, $rbf->getDimensions());

		$rbf = GaussianFunction::createZeroCentered(3);
		$this->assertEquals(9.118819655545162E-4, $rbf->calculate([1,2,3,4,5]));
		$this->assertEquals(1.3887943864964021E-11, $rbf->calculate([5,4,3,2,1]));
		$this->assertEquals(1.3887943864964021E-11, $rbf->calculate([5,4,3]));
